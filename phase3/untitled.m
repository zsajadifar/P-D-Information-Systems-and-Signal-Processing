...
for J = 1 : 10 + 1
    ...
    fft_noise([1, L / 2 + 1], :) = real(fft_noise([1, L / 2 + 1], :)); part1 = fft_noise(1: L / 2 + 1, :);
    for I=1:L/2+1
...
for J = 1 : nr_frames + 1
    win = [sqrt(hanning(sptf)), sqrt(hanning(sptf))];
    frame_signal = signal((sptf / 2)*(J-1) + 1 : (sptf / 2)*(J-1) + sptf, :).*(win); fft_signal =fft( frame_signal , L ) ;
    fft_signal([1, L / 2 + 1], :) = real(fft_signal([1, L / 2 + 1], :)); part1 = fft_signal(1: L / 2 + 1, :) ;
    ...
    for I=0:L/2
        ...
        %a_n = 1 / N;
        cc= [1; exp(zeta)];
        w = (noise_cor(:, :, I + 1) \ cc) / ((cc' / noise_cor(:, :, I + 1))*cc); 
        w = conj(w);
        testsignal(:, I + 1) = w;
    ...
    estimate_signal = [estimate_signal; 
    flipud(conj(estimate_signal(2 : end-1)))]; 
...
hold on
plot(linspace(1, fs, L), 10*log10(mean(abs(fft_estimate_signal_mat).^2, 2)), '*r'); title('Spectrum plots of in signal and signal after beamforming in dB')
...
% Polar plot
figure ;
f_center = linspace(0, fs-fs / (2 * L), L).'; polarfreq = [1500 , 2500];
