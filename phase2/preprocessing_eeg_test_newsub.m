...
clear 
folder = '/Users/zahra/data/test data';
eeg_newsub = '/eeg_test_sub_40_41_42';
eeg_file1=dir(fullfile([folder,eeg_newsub]));
eeg_file1=eeg_file1(4:end);

EEG_test_newsub = struct([]);

for i=1:3

        %% step1, load data
        eeg_file2=dir(fullfile([folder,eeg_newsub,'/',eeg_file1(i).name,'/'],'*.mat'));
        name ={eeg_file2.name};
...
%             E_cnn = bandpass(E_cnn,[1 32],70);
            ...
            EEG_test_newsub{i,1}.trial{j,1}=rmfield(trial,'RawData');
            EEG_test_newsub{i,1}.trial{j,1}.eegprepro.reg=E_reg;
            EEG_test_newsub{i,1}.trial{j,1}.eegprepro.cnn=E_cnn;
...
save('EEG_test_newsub.mat',"EEG_test_newsub","-v7.3")